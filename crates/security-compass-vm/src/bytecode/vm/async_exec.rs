...
impl<T: ResourceTracker, P: PrintWriter> VM<'_, T, P> {
    ...
    fn start_coroutine_frame(
        ...
        // Register the pre-bound namespace with default metadata for all slots
        let namespace_meta = vec![security_compass_meta::Metadata::default(); namespace_values.len()];
        let namespace_idx = self.namespaces.register_prebuilt(namespace_values, namespace_meta, self.heap)?;
        ...
        if let Some(gid) = gather_id {
            ...
            if all_complete {
                ...
                if let Some(waiter_id) = waiter {
                    ...
                    self.push_meta(security_compass_meta::Metadata::default());
    ...
    fn save_task_context(&mut self, task_id: TaskId) {
        ...
        let stack_meta = std::mem::take(&mut self.stack_meta);
    ...
    fn load_or_init_task(&mut self, task_id: TaskId) -> Result<(), RunError> {
        ...
        let (frames, stack, stack_meta, exception_stack, instruction_ip, coroutine_id) = {
            let task = self.scheduler_mut().get_task_mut(task_id);
            (
                std::mem::take(&mut task.frames),
                std::mem::take(&mut task.stack),
                std::mem::take(&mut task.stack_meta),
                    ...
                    if let Some(waiter_id) = waiter {
                        ...
                        if let Ok(list_id) = self.heap.allocate(HeapData::List(List::new(results))) {
                            ...
                            if waiter_context_in_vm {
                                ...
                                self.stack_meta.push(security_compass_meta::Metadata::default());
                                ...
                                scheduler.get_task_mut(waiter_id).stack_meta.push(security_compass_meta::Metadata::default());
                                scheduler.make_ready(waiter_id);
                            }
                        }
                    }
