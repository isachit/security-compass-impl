...
impl<T: ResourceTracker, P: PrintWriter> VM<'_, T, P> {
    ...
    fn call_sync_function(
        ...
        // Initialize parallel metadata for the namespace (all default)
        {
            let ns_meta = self.namespaces.get_mut(namespace_idx).mut_meta_vec();
            ns_meta.resize_with(func.namespace_size, security_compass_meta::Metadata::default);
        }
